...
train_ts <- window(daily_avg_45m_ts, start = c(2022,1), end= c(2022, 319))
valid_ts <- window(daily_avg_45m_ts, start= c(2022, 320))
#daily_avg_45m_ts
length(train_ts)
length(valid_ts)
```
...
```{r}
naive_model <- naive(train_ts, h=30)
...
```{r}
# show model fit 
autoplot(naive_predict) + ylab('Degrees Celsius') + coord_cartesian(xlim=c(2022.88,2022.95)) + autolayer(valid_ts, series='Actual', size=3) + autolayer(naive_predict$mean, color='black', series = 'Forecasted') + labs(title='Forecasted vs Actual')

```
```{r}
autoplot(naive_predict, series='Forecasted', ylab='Degrees Celsius') + autolayer(valid_ts, color='black', series= 'Actual') 
```
